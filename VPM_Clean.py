...
def Subspace_Iteration_Forecast(params_List,Data,InDc_stop,phi_t,guess,RHS,
                     optimizer_List,opt_state_List,Inner_Iter=5000,alpha=1.0,BS=2000,ls=None,l2=.01,Orthogonalize=True,
                     Nets_List=[],LossdJac=[],Lossd=[],dphi_star_List=[],Basis=False,Mem=False,n_QR_skip=0,Update_K=1,Train_Frac=1.0):
    ...
    for j in range(Inner_Iter):
        ...
        dc=LossdJac(params,Data[inds],phi_t[inds,:,0],guess[inds,:,0],
                    RHS[inds],InDc_stop[inds],alpha,l=ls[0])[0]
        opt_state,params=update(dc,opt_state,params)
    params_ans.append(params)
    opt_state_ans.append(opt_state) 
    
    Jacobi=dphi_star_List[0](Nets_List[0](Data,InDc_stop,params_ans[0])+guess[:,:,0])
    ...
    for j in range(Inner_Iter):
        ...
        dc=Lossd(params,Data[inds],phi_t[inds,:,1:],
                 InDc_stop[inds],guess[inds,:,1:],InDc_stop[inds]*0,
                 Jacobi[inds],alpha,l2=l2,l=ls[1])[0]
        dc[-1]=1*dc[-1]*Update_K
        dc[-2]=-1*dc[-2]
        opt_state,params=update(dc,opt_state,params)
    params_ans.append(params)
    opt_state_ans.append(opt_state)
    F=np.concatenate([dphi_star_List[0](Nets_List[0](Data,InDc_stop,params_ans[0])+guess[:,:,0])[:,:,None],dphi_star_List[1](Nets_List[1](Data,InDc_stop,params_ans[1])+guess[:,:,1:])],axis=-1)
    F[...,0]=Jacobi
    F=np.asarray(F,dtype=np.double)
    Norm=np.diag(np.mean(F[:,0]**2,axis=0))**.5
    Norm[0,0]=1
...
def Subspace_Iteration(params,Data,phi_t,
                     optimizer,opt_state,Inner_Iter=5000,alpha=1.0,BS=2000,
                     Lossd=None,orth=True,l=1.,l2=.01,Train_Frac=1.):
    ...
    F=np.asarray(F,dtype=np.double)
    Norms=np.diag(np.sum(F[:,0]**2,axis=0))**.5
    Q,R=np.linalg.qr(F[:,0,:])
...
def VAC(Basis):
    ...
    """
    X=Basis[:,:,1:]-np.sum(Basis[:,0,1:],axis=0)#mean subtract to remove trivial eigenvector
    C0=X[:,0].T@X[:,0]
    Ct=X[:,0].T@X[:,1]
    val,vec=scipy.linalg.eig(Ct,b=C0)
    inds=np.argsort(-np.conjugate(val)*val)
    val=np.asarray([1.]+list(val[inds]))
...
def LSTSQ_Triu(phi_t,l2=0):
    ...
    ans=np.zeros((N,N))
    #ans[0,0]=1
    ans[0,0]=(inv_spec(phi_t[:,0,0].T@phi_t[:,0,0])*(phi_t[:,0,0].T@phi_t[:,-1,0]))
    for i in range(1,N):
        Reg_Mat=np.eye(i+1)*l2
        Reg_Mat[-1,-1]=0
        ans[0:i+1,i]=inv_spec(phi_t[:,0,:i+1].T@phi_t[:,0,:i+1]+Reg_Mat)@(phi_t[:,0,:i+1].T@phi_t[:,-1,i])
    return ans
...
def FF_Subspace_Single(x,params):
    ...
    q=jnp.concatenate([jnp.ones_like(activations[:,0])[:,None],activations],axis=-1)
    return q
...
@jit
def relu(x):
    return jnp.maximum(0, x)+jnp.minimum(x, 0)*.01
#Use For Committor
@jit
def Loss_Jacobi_Sig(params,x,q_t,guess,RHS,InD,alpha,l=1.0):
    u=FF_ZBC(x,InD,params)+guess
    OP=q_t[:,-1]-RHS[:,-1]
    return jnp.mean(jax.nn.softplus(u[:,0]))-alpha*jnp.mean(OP*u[:,0])-(1-alpha)*jnp.mean(q_t[:,0]*u[:,0])
...
def Loss_Jacobi(params,x,q_t,guess,RHS,InD,alpha,l=1.0):
    u=FF_ZBC(x,InD,params)+guess
    OP=q_t[:,-1]+RHS[:,-1]
    return .5*jnp.mean(u[:,0]**2)-alpha*jnp.mean(OP*u[:,0])-(1-alpha)*jnp.mean(q_t[:,0]*u[:,0])


@jit
def Loss_Subspace(params,x,phi_t,alpha,l=1.0,l2=.01):
    phi=FF_Subspace(x,params)
    V=jnp.triu(params[-1])
    v=params[-2]
    norm=l*jnp.sum((V-jnp.diag(jnp.diag(V)))**2)
    norm2=l2*jnp.sum(2*v*(jnp.mean((phi[:,0])**2,axis=0)-1)-v**2)
    return .5*jnp.mean((phi[:,0]@V)**2)-alpha*jnp.mean(phi_t[:,-1]*(phi[:,0]@V))-(1-alpha)*jnp.mean(phi_t[:,0]*(phi[:,0]@V))+norm+norm2

@jit
def Loss_Subspace_Forecast(params,x,phi_t,InD,RHS,Guess,Jacobi,alpha,l=1.0,l2=.01):
    ...
    return .5*jnp.mean((F0)**2)-alpha*jnp.mean(phi_t[:,1]*(F0))-(1-alpha)*jnp.mean(phi_t[:,0]*(F0))+norm+norm2
